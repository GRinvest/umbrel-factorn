...
// creates umbrel-bitcoin-pow.conf
...
// creates bitcoin-pow.conf with includeconf=umbrel-bitcoin-pow.conf
async function generateBitcoinConfig(shouldOverwriteExistingFile = false) {
  ...
  // if bitcoin-pow.conf does not exist or should be overwritten, create it with includeconf=umbrel-bitcoin-pow.conf
  ...
  // if bitcoin-pow.conf exists but does not include includeconf=umbrel-bitcoin-pow.conf, add includeconf=umbrel-bitcoin-pow.conf to the top of the file
  if (!existingConfContents.includes(includeConfString)) {
    return await diskService.writePlainTextFile(constants.BITCOIN_CONF_FILEPATH, `${includeConfString}\n${existingConfContents}`);
  }

  // do nothing if bitcoin-pow.conf exists and contains includeconf=umbrel-bitcoin-pow.conf
}
...
// checks to see if umbrel-bitcoin-pow.conf is up to date, which we use to determine if we need to regenerate the config and restart bitcoind
async function isUmbrelBitcoinConfUpToDate(config) {
  ...
  // compare new config to existing umbrel-bitcoin-pow.conf
